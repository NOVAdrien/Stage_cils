# models/passive_chain_friction_with_activation_pickle_crit.py
...
from lagrangian.energy_builder import (
    ...
    energie_potentielle_ressorts_activation_critique
)
...
class PassiveChainFrictionWithActivationPickleCrit(BaseSystem):
    """
    Système de N barres rigides AVEC frottements et activation interne.
    """

    def __init__(self, N, masses, lengths, widths, stiffnesses, amplitudes, omega, k_spatial, phi, alpha, gamma, initial_conditions, epsilon, theta_crit):
        ...
        self.eps = sp.symbols(f'eps1:{N+1}')
        self.tc = sp.symbols(f'tc1:{N+1}')
        ...
        # Symboles pour les paramètres critiques
        self.eps = sp.symbols(f'eps1:{N+1}')  # epsilon values
        self.tc = sp.symbols(f'tc1:{N+1}')    # theta_crit values
        ...
        cache_file = f"rhs_modifie_num_N{N}_crit.pkl"
        ...
        if os.path.exists(cache_file) and os.path.getsize(cache_file) > 0:
            ...
            with open(cache_file, "rb") as f:
                rhs_with_friction_and_activation_pickle_crit = pickle.load(f)
            ...
            self.LM, self._rhs_func = generate_lagrangian_rhs(
                self.L, self.theta, self.t, [self.m, self.l, self.r, self.k, self.a, self.o, self.k_sp, self.phi_symb, self.eps, self.tc]
            )

            # On calcule le RHS symbolique
            rhs_with_friction_and_activation_pickle_crit = self.LM.rhs()

            # On ajoute les frottements :
            rhs_with_friction_and_activation_pickle_crit = ajoute_frottements_au_rhs(rhs_with_friction_and_activation_pickle_crit, self.Q_non_cons, self.M_list, self.N)

            # Sauvegarde du rhs_modifie numérisé
            with open(cache_file, "wb") as f:
                pickle.dump(rhs_with_friction_and_activation_pickle_crit, f)
        ...
        args = [self.t] + list(self.theta) + list(self.theta_dot) \
            ...
            + list(self.eps) + list(self.tc) \
            + list(self.alpha) + list(self.gamma)

        self._rhs_func = sp.lambdify(args, rhs_with_friction_and_activation_pickle_crit)
    ...
    def _build_lagrangian(self):
        ...
        self.Ep = energie_potentielle_ressorts_activation_critique(
            self.N, self.theta, self.t, self.k, self.l, self.amplitudes, self.omega, self.k_spatial, self.phi, self.eps, self.tc
        )
    ...
    def solve(self, t_span, t_eval=None, method='RK45'):
        ...
        def rhs(t_num, y):
            ...
            inputs = [t_num] + list(theta_vals) + list(theta_dot_vals) \
                ...
                + list(self.amplitudes) + list(self.omega) + list(self.k_spatial) + list(self.phi) \
                + list(self.epsilon) + list(self.theta_crit) \
